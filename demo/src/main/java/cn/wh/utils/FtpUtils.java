...
public class FtpUtils {
     ...
     * 获取目录下所有的文件名称
     *
     * @param ftpPath 指定的ftp目录
     * @return 文件列表,或者null
     */
    private FTPFile[] getFileList(String ftpPath) {
        try {
    ...
    public boolean changeDir(String ftpPath) {
        ...
        try {
            ...
            if (ftpPath.indexOf('/') == -1) {
                // 只有一层目录
                ftpClient.changeWorkingDirectory(new String(ftpPath.getBytes(), Charset.defaultCharset()));
            } else {
                // 多层目录循环创建
                String[] paths = ftpPath.split("/");
                for (int i = 0; i < paths.length; i++) {
                    ftpClient.changeWorkingDirectory(new String(paths[i].getBytes(), Charset.defaultCharset()));
                }
            }
            return true;
        } catch (Exception e) {
    ...
    public boolean mkDir(String ftpPath) {
        ...
        try {
            ...
            if (ftpPath.indexOf('/') == -1) {
                // 只有一层目录
                ftpClient.makeDirectory(new String(ftpPath.getBytes(), Charset.defaultCharset()));
                ftpClient.changeWorkingDirectory(new String(ftpPath.getBytes(), Charset.defaultCharset()));
            } else {
                // 多层目录循环创建
                String[] paths = ftpPath.split("/");
                for (int i = 0; i < paths.length; i++) {
                    ftpClient.makeDirectory(new String(paths[i].getBytes(), Charset.defaultCharset()));
                    ftpClient.changeWorkingDirectory(new String(paths[i].getBytes(), Charset.defaultCharset()));
                }
            }
            return true;
    ...
    /**
     * 上传文件到FTP服务器
     * @param localDirectoryAndFileName 本地文件目录和文件名:本地文件地址
     * @param ftpFileName 上传到服务器使用的文件名
    ...
    public boolean upload(String localDirectoryAndFileName, String ftpFileName, String ftpDirectory) {
        ...
        if (ftpClient != null) {
            ...
            try {
                ...
                ftpClient.setControlEncoding(Charset.defaultCharset().toString());
                // 设置文件类型（二进制）
    ...
    public boolean get(String ftpDirectoryAndFileName, String localDirectoryAndFileName) {
        ...
        try {
            ...
            ftpClient.retrieveFile(new String(fileName.getBytes(), Charset.defaultCharset()),
    ...
    public static void main(String[] args) {
        FtpUtils f = new FtpUtils("ip", 21, "username", "mima");
        try {
            if(f.open()) {
                System.out.println("连接成功");
                String fileName = "WechatIMG22.jpeg";
                //上传
                boolean b = f.upload("/Users/wanghao/Desktop/WechatIMG22.jpeg", fileName, "/wwwroot/resource/mmall");
                System.out.println(b);

//                //遍历
//                FTPFile[] list = f.getFileList("/resource/mmall");
//                for(FTPFile file : list) {
//                    String name = file.getName();
//                    System.out.println("--" + new String(name.getBytes("iso-8859-1"), Charset.defaultCharset()));
//                }
//
//                //只遍历指定目录下的文件名
//                String[] names = f.getFileNameList("/resource/mmall");
//                for(String name : names) {
//                    System.out.println(new String(name.getBytes("iso-8859-1"), Charset.defaultCharset()));
//                }
//
//                //下载
//                boolean b = f.get("/resource/mmall/测试2.txt", "/Users/wanghao/Desktop/10086.txt");
//                System.out.println(b);

                //删除
//                String ftpDirAndFileName = "/resource/mmall/测试2.txt";
